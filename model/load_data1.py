...
def load_data(test_percentage=0.3, momentum_window=1, news_per_hour = 10):
    ...
    for i, row in x.iterrows():
        x.at[i,'PUBLICATION_DATE'] =datetime.strptime(x['PUBLICATION_DATE'][i], '%Y-%m-%d %H:%M:%S +%f')
        ...
        # Replicazione news se sono troppo poche
            ...
            if(len(timeSlotX) < news_per_hour):
                ...
                while(len(timeSlotX) < news_per_hour):
                    timeSlotX.append(timeSlotX[i%numNews])
                # prevSlot = len(X)-1
