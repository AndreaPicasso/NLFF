...
class MyModel():
	...
	def create_placeholders():
	    X = tf.placeholder(tf.float32, shape=(None, None, word_count_dim), name='X')
	...
	def lstm_cell(num_lstm_units):
		lstm = tf.contrib.rnn.BasicLSTMCell(num_lstm_units)
		return lstm

	def forward_propagation(X, num_lstm_units = 100, num_lstm_layers = 1):
   
		print(X.shape)
		lstm_network = tf.contrib.rnn.MultiRNNCell([MyModel.lstm_cell(num_lstm_units) for _ in range(num_lstm_layers)])
		outputs,_ = tf.nn.dynamic_rnn(lstm_network, X, dtype="float32")
	...
	def model(X_train, Y_train, X_test, Y_test, learning_rate = 0.009,
	          num_epochs = 25, minibatch_size = 128,
	    ...
	    prediction = MyModel.forward_propagation(X, num_lstm_units, num_lstm_layers)
...
MyModel.model(X_train, Y_train, X_test, Y_test,  minibatch_size = batch_size, learning_rate = learning_rate, num_lstm_units = num_lstm_units, num_lstm_layers = num_lstm_layers)
