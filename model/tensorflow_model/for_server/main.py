    ...
    def load_data(ticker, momentum_window=30, X_window_average=None, news_per_hour = 10, newsTimeToMarket = 0, set_verbosity=True):
        ...
        while(x['PUBLICATION_DATE'][i] < finalDate and y['DATE'][j] < finalDate ):
            ...
            if(len(timeSlotX) > news_per_hour):
                ...
                #timeSlotX = timeSlotX[-news_per_hour:]
...
class MyModel():
    ...
    def __init__(self, num_lstm_units, num_lstm_layers, news_per_hour, learning_rate = 0.009,
              num_epochs = 30, minibatch_size = 128, dropout_prob=0):
    ...
    def create_placeholders(self):
        ...
        return X, Y, lstm_state_placeholder,dropout_prob



    def forward_propagation(self, X,dropout_prob, init_state = None):
    ...
    def run(self, X_train, Y_train, X_dev, Y_dev, set_verbosity=True):
        ...
        X, Y, lstm_state_placeholder, dropout_prob = self.create_placeholders()

        # Forward propagation: Build the forward propagation in the tensorflow graph
        prediction, lstm_next_state = self.forward_propagation(X,dropout_prob, lstm_state_placeholder)
          ...
          # Do the training loop
            for epoch in range(self.num_epochs):
                ...
                for minibatch in minibatches:
                    ...
                    _ , temp_cost, lstm_state = sess.run([optimizer, cost, lstm_next_state], feed_dict={X: minibatch_X, Y: minibatch_Y, lstm_state_placeholder: lstm_state, dropout_prob:self.dropout_prob})
                ...
                if  epoch % 1 == 0:
                    ...
                    trainCost, lstm_temp_state = sess.run([cost, lstm_next_state], feed_dict={X: np.asarray(X_train), Y: np.asarray(Y_train).reshape((len(Y_train), 1)), lstm_state_placeholder: lstm_temp_state,  dropout_prob:0})
                    costs_train.append(trainCost)
                    devCost = sess.run(cost, feed_dict={X: np.asarray(X_dev), Y: np.asarray(Y_dev).reshape((len(Y_dev), 1)), lstm_state_placeholder: lstm_temp_state,  dropout_prob:0})
                    costs_dev.append(devCost)

                    lstm_temp_state  = self.get_initial_state()
                    trainAccuracy, lstm_temp_state = sess.run([accuracy, lstm_next_state], feed_dict={X: np.asarray(X_train), Y: np.asarray(Y_train).reshape((len(Y_train), 1)),lstm_state_placeholder: lstm_temp_state,  dropout_prob:0})
                    devAccuracy = sess.run(accuracy, feed_dict={X: np.asarray(X_dev), Y: np.asarray(Y_dev).reshape((len(Y_dev), 1)),lstm_state_placeholder: lstm_temp_state,  dropout_prob:0})
                    yhat = sess.run(prediction, feed_dict={X: np.asarray(X_dev), Y: np.asarray(Y_dev).reshape((len(Y_dev), 1)),lstm_state_placeholder: lstm_temp_state,  dropout_prob:0})
    ...
    def runWithEarlyStopping(self, X_train, Y_train, X_dev, Y_dev, X_test=None, Y_test=None, set_verbosity=True):
        ...
        # Create Placeholders of the correct shape
        X, Y, lstm_state_placeholder, _ = self.create_placeholders()
...
#                   '\t LSTMUnits: '+str(numLSTMUnits)+'\t lr: '+str(learning_rate)+'\t miniBatch: '+str(minibatch_size))
                ...
                for fold in folds:
                    ...
                    (X_train, Y_train), (X_dev, Y_dev) = fold
                    model = MyModel(num_lstm_units=numLSTMUnits, num_lstm_layers=1, news_per_hour = news_per_hour,
                                learning_rate = learning_rate, num_epochs = 30, minibatch_size = minibatch_size, dropout_prob=0.3)
                
                    (train_accuracy, dev_accuracy), _ = model.run(X_train,Y_train,X_dev,Y_dev, set_verbosity=False)
...
Data.load_data(ticker= 'AAPL', X_window_average=30, news_per_hour=30,
                               momentum_window=30, newsTimeToMarket = 0, set_verbosity=False)
(X_train, Y_train), (X_test, Y_test) = Data.get_train_test_set()
model = MyModel(num_lstm_units=300, num_lstm_layers=1, news_per_hour=30,
