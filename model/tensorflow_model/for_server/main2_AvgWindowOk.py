...
class Data():
    ...
    def load_data(ticker='AAPL', momentum_window=30, X_window_average=None, newsTimeToMarket = 0, set_verbosity=True):
        ...
        while(y['DATE'][j] < finalDate ):
            ...
            timeSlotX = [temp['CONSTRAINING'].tolist(),temp['LITIGIOUS'].tolist(),temp['NEGATIVE'].tolist(),temp['POSITIVE'].tolist(),temp['UNCERTAINTY'].tolist()]
...
class MyModel():
    ...
    def runWithEarlyStopping(self, X_train, Y_train, X_dev, Y_dev, X_test=None, Y_test=None, set_verbosity=True):
        ...
        if(type(X_test) != type(None) and type(Y_test) != type(None)):
            ...
            return (accuracy_train, accuracy_dev), []
    ...
    def modelSelectionFixedTTM(ticker='AAPL', iterations = 20, learning_rate = 0.001, minibatch_size = 512):
        ...
        for newsTimeToMarketIndex in range(0, len(Ttm_range)):
            ...
            for i in range(0,iterations):
                ...
                Data.load_data(ticker= ticker,momentum_window=30,X_window_average=X_window_average, newsTimeToMarket = newsTimeToMarket, set_verbosity=False)
...
#                 print('( train_fold_variance: '+str(np.var(np.asarray(k_fold_train_accuracy)))+ ', dev_fold_variance: '+str(np.var(np.asarray(k_fold_dev_accuracy)))+' )')
        ...
        for newsTimeToMarketIndex in range(0, len(Ttm_range)):
            newsTimeToMarket = Ttm_range[newsTimeToMarketIndex]
            Data.load_data(ticker= ticker,momentum_window=30,  X_window_average=best_X_window_average[newsTimeToMarketIndex], newsTimeToMarket = newsTimeToMarket, set_verbosity=False)
...
model = MyModel(num_lstm_units=num_lstm_units, num_lstm_layers=num_lstm_layers, news_per_hour = train_x.shape[1],
                learning_rate = learning_rate, num_epochs = 50, minibatch_size = batch_size)
