...
#                 (train_x[index+samples_per_fold:index+samples_per_fold+dev_num_points], train_y[index+samples_per_fold:index+samples_per_fold+dev_num_points])))
    ...
    def load_data(ticker, momentum_window=30, X_window_average=None, news_per_hour = 10, newsTimeToMarket = 0, set_verbosity=True):
        ...
        y.drop(np.arange(momentum_window), inplace=True)
        y = y.reset_index(drop=True)
...
#                 print('( train_fold_variance: '+str(np.var(np.asarray(k_fold_train_accuracy)))+ ', dev_fold_variance: '+str(np.var(np.asarray(k_fold_dev_accuracy)))+' )')
        ...
        for newsTimeToMarketIndex in range(0, len(Ttm_range)):
            ...
            MCCs.append((tp*tn -fp*fn)/((tp+fp)*(tp+fn)*(tn+fp)*(tn+fn)))
...
# Data.load_data(ticker= 'GOOGL', news_per_hour = 30, X_window_average=30,
...
# print(len(M_test))
# print(np.sum(np.asarray(M_test)))
# print(len(M_train))
# print(np.sum(np.asarray(M_train)))
# b = [1 if (Y_test[i]==1 and M_test[i]) else 0 for i in range(len(Y_test))]
# b += [1 if (Y_train[i]==1 and M_train[i]) else 0 for i in range(len(Y_train))]

# a = [1 if (Y_test[i]==0 and M_test[i]) else 0 for i in range(len(Y_test))]
# a += [1 if (Y_train[i]==0 and M_train[i]) else 0 for i in range(len(Y_train))]
...
for tic in tickers:
    ModelSelection.modelSelectionFixedTTM(ticker=tic, iterations = 15, learning_rate = 0.001, minibatch_size = 512)
