...
#                 (train_x[index+samples_per_fold:index+samples_per_fold+dev_num_points], train_y[index+samples_per_fold:index+samples_per_fold+dev_num_points])))
    ...
    def load_data(ticker, momentum_window=30, newsTimeToMarket = 0, X_window_average=None, set_verbosity=True):
    ...
    def modelSelectionFixedTTM(ticker='AAPL', iterations = 20, learning_rate = 0.001, minibatch_size = 512):
        ...
        for newsTimeToMarketIndex in range(0, len(Ttm_range)):
            ...
            for i in range(0,iterations):
                ...
                Data.load_data(ticker= ticker,momentum_window=30, newsTimeToMarket = newsTimeToMarket,X_window_average=X_window_average, set_verbosity=False)
...
#                 print('( train_fold_variance: '+str(np.var(np.asarray(k_fold_train_accuracy)))+ ', dev_fold_variance: '+str(np.var(np.asarray(k_fold_dev_accuracy)))+' )')
        ...
        for newsTimeToMarketIndex in range(0, len(Ttm_range)):
            newsTimeToMarket = Ttm_range[newsTimeToMarketIndex]
            Data.load_data(ticker= ticker,momentum_window=30, newsTimeToMarket = newsTimeToMarket,
                X_window_average=best_X_window_average[newsTimeToMarketIndex], set_verbosity=False)
...
Data.load_data(ticker= 'AAPL',momentum_window=30, newsTimeToMarket = 0, X_window_average = 30, set_verbosity=False)
