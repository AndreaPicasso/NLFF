...
class Data():
    ...
    def get_train_test_set(test_percentage=0.2):
    ...
    def get_cross_validation_train_dev_set(test_percentage=0.2, k_fold = 10,  dev_num_points=200):
...
#                 (train_x[index+samples_per_fold:index+samples_per_fold+dev_num_points], train_y[index+samples_per_fold:index+samples_per_fold+dev_num_points])))
    ...
    def load_data(ticker, momentum_window=30, newsTimeToMarket = 0, set_verbosity=True):
        ...
        #     z.append((y['close'][i+momentum_window] - y['close'][i])/y['close'][i])
        ...
        weekHours = 49 # I suppose a window of 1 week on the past to take in account news
        currentDate = initDate
        while(y['DATE'][j] < finalDate ):
            Y.append(y['close'][j])
            Z.append(z[j])
            temp = x[x['PUBLICATION_DATE'] < y['DATE'][j]]
            temp = temp[temp['PUBLICATION_DATE'] > y['DATE'][j] - timedelta(days=15)]
...
class MyModel():
    ...
    def create_placeholders(self):
        ...
        keep_prob = tf.placeholder(tf.float32, shape=(), name='is_training')
        
        return X, Y, lstm_state_placeholder, threshold_mask, keep_prob



    def forward_propagation(self, X, keep_prob, init_state = None):
        ...
        drop1 = tf.nn.dropout(outputs,keep_prob, name='drop1')
        prediction = tf.layers.dense(drop1, 1, activation=tf.nn.relu)
        drop2 = tf.nn.dropout(prediction, keep_prob, name='drop2')
        prediction = tf.layers.dense(drop2, 1, activation=tf.nn.sigmoid)
    ...
    def run(self, X_train, Y_train, M_train, X_dev, Y_dev, M_dev, set_verbosity=True):
        ...
        X, Y, lstm_state_placeholder, threshold_mask, keep_prob = self.create_placeholders()

        # Forward propagation: Build the forward propagation in the tensorflow graph
        prediction, lstm_next_state = self.forward_propagation(X, keep_prob, lstm_state_placeholder)
          ...
          # Do the training loop
            for epoch in range(self.num_epochs):
                ...
                for minibatch in minibatches:
                    ...
                    _ , temp_cost, lstm_state = sess.run([optimizer, cost, lstm_next_state], feed_dict={X: minibatch_X, Y: minibatch_Y, lstm_state_placeholder: lstm_state, threshold_mask:minibatch_M, keep_prob:np.asarray(0.8)})
                ...
                if  epoch % 1 == 0:
                    ...
                    trainCost, lstm_temp_state = sess.run([cost, lstm_next_state], feed_dict={X: np.asarray(X_train), Y: np.asarray(Y_train).reshape((len(Y_train), 1)), lstm_state_placeholder: lstm_temp_state,threshold_mask:np.asarray(M_train).reshape((len(M_train), 1)), keep_prob:np.asarray(1)})
                    costs_train.append(trainCost)
                    devCost = sess.run(cost, feed_dict={X: np.asarray(X_dev), Y: np.asarray(Y_dev).reshape((len(Y_dev), 1)), lstm_state_placeholder: lstm_temp_state, threshold_mask:np.asarray(M_dev).reshape((len(M_dev), 1)), keep_prob:np.asarray(1)})
                    costs_dev.append(devCost)

                    lstm_temp_state  = self.get_initial_state()
                    trainAccuracy, lstm_temp_state = sess.run([accuracy, lstm_next_state], feed_dict={X: np.asarray(X_train), Y: np.asarray(Y_train).reshape((len(Y_train), 1)),lstm_state_placeholder: lstm_temp_state, threshold_mask:np.asarray(M_train).reshape((len(M_train), 1)), keep_prob:np.asarray(1)})
                    devAccuracy = sess.run(accuracy, feed_dict={X: np.asarray(X_dev), Y: np.asarray(Y_dev).reshape((len(Y_dev), 1)),lstm_state_placeholder: lstm_temp_state, threshold_mask:np.asarray(M_dev).reshape((len(M_dev), 1)), keep_prob:np.asarray(1)})
                    yhat = sess.run(prediction, feed_dict={X: np.asarray(X_dev), Y: np.asarray(Y_dev).reshape((len(Y_dev), 1)),lstm_state_placeholder: lstm_temp_state, keep_prob:1})
    ...
    def modelSelectionFixedTTM(ticker='AAPL', iterations = 20, learning_rate = 0.001, minibatch_size = 512):
        ...
        for newsTimeToMarketIndex in range(0, len(Ttm_range)):
            ...
            for i in range(0,iterations):
                numLSTMUnits = np.asscalar(np.random.random_integers(200, 800))
