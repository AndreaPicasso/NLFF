...
#                 (train_x[index+samples_per_fold:index+samples_per_fold+dev_num_points], train_y[index+samples_per_fold:index+samples_per_fold+dev_num_points])))
    ...
    def load_data(ticker, momentum_window=30, newsTimeToMarket = 0, set_verbosity=True):
        X_path = 'embeddings/'+str(ticker)+'_EMBEDDING.json'
        ...
        for i, row in x.iterrows():
            x.at[i,'PUBLICATION_DATE'] =datetime.strptime(x['PUBLICATION_DATE'][i], '%Y-%m-%d %H:%M:%S +0000') + timedelta(hours=newsTimeToMarket)
            if type(x['EMBEDDING'][i]) == type("asd"):
                x.drop(i, inplace=True)
            #x.at[i, 'EMBEDDING']= x['EMBEDDING'][i]
        ...
        while(y['DATE'][j] < finalDate ):
            Y.append(y['close'][j])
            Z.append(z[j])
            #X.append(np.asarray(x['EMBEDDING'][(x['PUBLICATION_DATE'] < y['DATE'][j]) & (x['PUBLICATION_DATE'] > y['DATE'][j] - timedelta(days=7))].tolist()).transpose().tolist())
            X.append(x['EMBEDDING'][(x['PUBLICATION_DATE'] < y['DATE'][j]) & (x['PUBLICATION_DATE'] > y['DATE'][j] - timedelta(days=7))].tolist())
...
#                 numLSTMUnits = np.asscalar(np.random.random_integers(100, 800))
...
#         print('Training optimal model...')
        ...
        for newsTimeToMarketIndex in range(0, len(Ttm_range)):
            ...
            model = MyModel(num_lstm_units=300, num_lstm_layers=1,
...
for tic in tickers:
    ModelSelection.modelSelectionFixedTTM(ticker=tic, iterations = 2, learning_rate = 0.001, minibatch_size = 512)
