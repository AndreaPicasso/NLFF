...
class Data():
    ...
    def load_data(ticker='AAPL', momentum_window=28, newsTimeToMarket = 0, set_verbosity=True):
        ...
        for i in range(30,len(Xtemp)):
            xt = list()
            xt.append(Xtemp[i])
            for window in np.arange(5, 31, 5): #Inserisco le medie 5,10,15,..30
                xt.append(np.average(Xtemp[i-window:i], axis=0))
            X.append(xt)
...
# ticker='AAPL'
    ...
    for newsTimeToMarket in Ttm_range:
        ...
        plt.savefig('QuantileRegressionExperimentsAttentionOnAvg/'+ticker+'_'+str(newsTimeToMarket)+'.jpg')
