...
'''
# Aggiunge una nuova colonna al dataframe, 'conceptColumnName' contenente i concetti estratti
'''

def parse(dfToParse, textColumnName , conceptColumnName):
	dfToParse[conceptColumnName] = ''

	start_time = time.time()
	count0 = 1
	for rowIndex, row in dfToParse.iterrows():

		print(rowIndex)



		senten = row[textColumnName]
		...
		dfToParse[conceptColumnName].at[rowIndex] = final_concepts
...
def parseToFile(fileToParse, fileOutput):
	start_time = time.time()
	input_file = open(fileToParse,'r')									# Input file containing sentences to parse
	output_file = open(fileOutput,'a+')
	count0 = 1
